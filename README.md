...
- Throughout the repository, the name `jackbot_lab` only serves as an example and we provide a script to rename all the references to it automatically:

```bash
# Enter the repository
cd IsaacLabExtensionTemplate
# Rename all occurrences of jackbot_lab (in files/directories) to your_fancy_extension_name
...
We provide an example UI extension that will load upon enabling your extension defined in `source/jackbot_lab/jackbot_lab/ui_extension_example.py`.
...
```json
{
    "python.analysis.extraPaths": [
        "<path-to-ext-repo>/source/jackbot_lab"
    ]
}
```
